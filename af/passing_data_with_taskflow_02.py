...
@dag(dag_id='passing_data_with_taskflow_02',
...
def passing_data_with_taskflow_api():
    ...
    @task
    def display_result(code, status):
        print(f"Return Code: {code} & {status}")



    result_code = get_code()
    result_status = get_status(result_code)
    display_result(result_status['code'], result_status['status'])
